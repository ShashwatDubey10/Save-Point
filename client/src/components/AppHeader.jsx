...
=======
    <>
      {/* Mobile-first header */}
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white/95 dark:bg-dark-900/95 backdrop-blur-lg border-b border-gray-200 dark:border-white/10 safe-padding-top">
...
=======
            ...
            <div className="flex items-center gap-2 lg:hidden">
              ...
              <button
                ...
                className="touch-target flex items-center justify-center w-10 h-10 sm:w-11 sm:h-11 rounded-lg glass hover:bg-gray-100 dark:hover:bg-white/10 transition-colors"
                aria-label="Menu"
              >
            ...
            <div className="hidden lg:flex items-center gap-3">
              ...
              <div className="glass px-4 py-2.5 rounded-xl h-[45px] cursor-pointer" title={`${xpToNextLevel} XP to Level ${level + 1}`}>
                <div className="flex items-center gap-2 min-w-[180px]">
                  <span className="text-yellow-400 text-lg">⭐</span>
                  <div className="flex-1">
                    <div className="text-xs text-gray-600 dark:text-gray-400 mb-1">Level {level} • {xpToNextLevel} XP to next</div>
              ...
              {/* User Avatar & Info */}
              <Link to="/profile" className="flex items-center gap-3 glass px-3 py-2.5 rounded-xl hover:bg-gray-100 dark:hover:bg-white/10 transition-all h-[45px]">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary-500 to-primary-700 flex items-center justify-center text-white font-bold text-sm">
                  {user?.username?.charAt(0).toUpperCase() || 'U'}
                </div>
                <div>
                  <div className="text-sm font-medium text-gray-900 dark:text-white leading-none">{user?.username || 'User'}</div>
                </div>
              </Link>
              ...
              <button
                onClick={toggleTheme}
                className="flex items-center justify-center w-[45px] h-[45px] glass rounded-xl hover:bg-gray-100 dark:hover:bg-white/10 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors"
              ...
              <button
                onClick={handleLogout}
                className="flex items-center gap-2 glass px-4 py-2.5 hover:bg-gray-100 dark:hover:bg-white/10 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white rounded-xl transition-colors h-[45px]"
                title="Logout"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
          ...
          {showMobileMenu && (
            <div className="lg:hidden pb-3 pt-2 border-t border-gray-200 dark:border-white/10 mt-2 animate-fade-in-up">
              {/* Level & XP - Mobile */}
              <div className="glass rounded-xl p-3 mb-3">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-yellow-400 text-lg">⭐</span>
                  <div className="text-sm font-semibold text-gray-900 dark:text-white">Level {level}</div>
                  <span className="text-xs text-gray-600 dark:text-gray-400">• {totalXP} XP</span>
                </div>
                ...
                <div className="text-xs text-gray-600 dark:text-gray-400 mt-1.5">{xpToNextLevel} XP to Level {level + 1}</div>
              </div>

              {/* User Info - Mobile */}
              <Link
                to="/profile"
                className="flex items-center gap-3 glass rounded-xl p-3 mb-2 hover:bg-gray-100 dark:hover:bg-white/10 transition-colors touch-target"
              ...
              <button
                onClick={toggleTheme}
                className="w-full flex items-center gap-3 glass rounded-xl p-3 mb-2 hover:bg-gray-100 dark:hover:bg-white/10 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors touch-target"
              >
                {theme === 'dark' ? (
                  <>
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button
                ...
                className="w-full flex items-center gap-3 glass rounded-xl p-3 hover:bg-red-500/10 text-gray-600 dark:text-gray-400 hover:text-red-400 transition-colors touch-target"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                </svg>
